...
class olam_download_performance_details_widget extends WP_Widget {
	...
	function widget($args, $instance)
	{
		...
		if(function_exists("fw_ext_feedback_stars_get_post_rating")){
			...
			$commentcount=fw_ext_feedback_stars_get_post_rating($postID);
			$commentcount=$commentcount['count'];
			$commentlink='#comments_area';
		}
		elseif ( class_exists( 'EDD_Reviews' ) ) {
				$starRating = edd_reviews()->average_rating(false);
				$starRating=(round($starRating*2))*10;

				$commentcount=edd_reviews()->count_reviews();
				$commentlink='#edd-reviews';
			} else {
				$commentcount=olam_get_comment_count($postID);
				$commentlink='#comments_area';
			}
		...
		<ul class="milestones">
			...
			<li><i class="demo-icons icon-comment-alt"></i><?php echo $commentcount; ?><span> <a href="<?php echo $commentlink;?>"><?php esc_html_e("Comments","olam"); ?></a></span></li>
			<li>
