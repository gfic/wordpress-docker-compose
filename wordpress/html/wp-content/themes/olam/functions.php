              ...
              foreach ($cartContents as $cartContentsKey => $cartContentsValue) { 
                ...
                <li>
                  ...
                  <a href="<?php echo get_permalink($cartContentsValue['id']); ?>">
                    <?php
                    $featImage=null;
                    $realfeatImage = wp_get_attachment_image_src( get_post_thumbnail_id( $cartContentsValue['id'] ), 'olam-product-thumb-small' );
                    ...
                    <?php if(isset($featImage)&&strlen($featImage)>0) { 
                      ...
                      ?> 
                      <img src="<?php echo esc_url($featImage);  ?>" class="attachment-shop_thumbnail wp-post-image" alt="<?php echo esc_html(get_the_title($cartContentsValue['id'])); ?>">
                      <?php } elseif (isset($realfeatImage)) { ?>
                        <img src="<?php echo esc_url($realfeatImage); ?>" alt="<?php echo esc_html(get_the_title($cartContentsValue['id'])); ?>">
                      <?php } else {?>
...
function olam_get_terms_dropdown() {
    $values = array(
      'show_option_all' => esc_html__('All','olam'),
      'orderby' => 'name',
      'order' => 'ASC',
      'echo' => 1,
      'hide_empty' => 1,
      'show_count' => 1,
      'selected' => $kat = get_query_var( 'download_cat' ),
      'name' => 'download_cat',
      'hierarchical'  => 1,
      'id' => '',
      'value_field' => 'name',
      'class' => 'download_cat_filter',
      'taxonomy' => 'download_category'
     );
    $categories = wp_dropdown_categories($values);
}
...
function olam_filter_theme_fw_ext_backups_demos($demos) {
    $demos_array = array(
        'olam_demo_1_2' => array(
            ...
            'screenshot'   => 'https://themes.layero.com/olamwp/demo-data/demo12-main/screenshot.png',
            'preview_link' => 'https://themes.layero.com/olamwp/',
        ),
        'olam_demo_3' => array(
            'title'        => esc_html__('demo3-Services', 'olam'),
            'screenshot'   => 'https://themes.layero.com/olamwp/demo-data/demo3-services/screenshot.png',
            'preview_link' => 'https://themes.layero.com/olamwp3/',
        ),
        'olam_demo_4' => array(
            'title'        => esc_html__('demo4-Booking', 'olam'),
            'screenshot'   => 'https://themes.layero.com/olamwp/demo-data/demo4-booking/screenshot.png',
            'preview_link' => 'https://themes.layero.com/olamwp4/',
        ),
    );

    $download_url = 'https://themes.layero.com/olamwp/demo-data/index.php';
...
function st_custom_admin_notice() {
  $st_th_info = wp_get_theme(); 
  $currentversion = str_replace('.','',(esc_html( $st_th_info->get('Version') )));
  $isitdismissed = 'st_notice_dismissed'.$currentversion;
  if ( !get_user_meta( get_current_user_id() , $isitdismissed ) ) { ?>
  <div class="notice notice-success is-dismissible st_notice" data-dismissible="disable-done-notice-forever">
    <div>
      <p> 
      <?php _e('Thank you for updating to the latest version of ','olam'); ?>
      <?php echo esc_html( $st_th_info->get('Name') );?> - 
      <?php echo esc_html( $st_th_info->get('Version') );
       ?>
      <?php _e('theme. Check out the best ', 'olam'); ?>
      <a href="<?php echo esc_url( 'https://layero.com/eddaddons?utm_source=ThemeAdmin&utm_medium=UpdateMsg&utm_campaign=Olam');?>" target="_blank" aria-label="Dismiss this message">
        <strong><?php _e('EDD Addons','olam');?></strong>
      </a>
      <?php _e('and extend your site features!', 'olam'); ?>
      <a href="?st-notice-dismissed<?php echo $currentversion;?>">Dismiss this message</a>
      </p>
    </div>
...
add_action('admin_notices', 'st_custom_admin_notice');

function st_notice_dismissed() {
  $st_th_info = wp_get_theme(); 
  $currentversion = str_replace('.','',(esc_html( $st_th_info->get('Version') )));
  $dismissurl = 'st-notice-dismissed'.$currentversion;
  $isitdismissed = 'st_notice_dismissed'.$currentversion;
    $user_id = get_current_user_id();
    if ( isset( $_GET[$dismissurl] ) )
        add_user_meta( $user_id, $isitdismissed, 'true', true );
}
add_action( 'admin_init', 'st_notice_dismissed' );


// Hide Brizy page builder upsell admin notice
add_action('admin_head', 'hide_brizy_admin_notice');
function hide_brizy_admin_notice() {
  echo '<style>
    .fw-brz-dismiss {
      display:none!important;
    }
    .appearance_page_optionsframework #header .notice {display:none!important;}
  </style>';
}
