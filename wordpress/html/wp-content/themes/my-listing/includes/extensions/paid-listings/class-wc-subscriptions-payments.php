			...
			if ( $product->is_type( array( 'job_package_subscription' ) ) && $subscription->get_user_id() && ! isset( $item['switched_subscription_item_id'] ) ) {
				...
				foreach ( $listing_ids as $listing_id ) {
					if ( in_array( get_post_status( $listing_id ), array( 'pending_payment', 'expired' ), true ) ) {
						...
						$listing_data = array(
							'ID'            => $listing_id,
							'post_status'   => ( in_array( $old_status, ['pending', 'publish'] ) ? $old_status : ( get_option( 'job_manager_submission_requires_approval' ) ? 'pending' : 'publish' ) ),
						);
				...
				if ( ! empty( $item['job_id'] ) && ! get_option( 'job_manager_submission_requires_approval' ) && 'listing' === $subscription_type ) {
