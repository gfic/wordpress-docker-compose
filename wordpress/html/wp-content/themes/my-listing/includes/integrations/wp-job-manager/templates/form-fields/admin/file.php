...
<div class="file-upload-field <?php echo esc_attr( $is_multiple ? 'multiple-uploads' : 'single-upload' ) ?> form-group-review-gallery">
	<div class="uploaded-files-list review-gallery-images">
		...
		<input type="hidden" class="input-text" name="<?php echo esc_attr( $field_name ) ?>" value="">
		...
		<div class="job-manager-uploaded-files">
			<?php foreach ( $uploaded_files as $file ): ?>
				...
				<div class="uploaded-file">
					<?php if ( $is_image ): ?>
						<span class="uploaded-image-preview" style="background-image: url('<?php echo esc_url( job_manager_get_resized_image( $image_src, 'medium' ) ?: $image_src ) ?>');">
						</span>
					<?php else: ?>
					...
					<input type="hidden" class="input-text" name="<?php echo esc_attr( $field_name ) ?>" value="<?php echo esc_attr( $file ) ?>">
				</div>
			<?php endforeach ?>
		</div>
	</div>
